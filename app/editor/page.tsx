 ...
 *   3. Replace all hardcoded UI strings with t() calls
 *   4. Toast messages use t() with optional vars (e.g. file name, format)
 */
...
export default function EditorPage() {
  ...
  const onFileSelect = useCallback(
    async (file: File) => {
      ...
      if (!ok) {
        addToast('error', t('editor_toast_invalid')); // ← t()
      } else {
        addToast('info', t('editor_toast_loaded', { name: file.name })); // ← t() with var
      }
      return ok;
    },
  ...
  const handleApplyCompression = async () => {
    ...
    addToast('success', t('editor_toast_compressed')); // ← t()
  };

  const handleApplyCrop = async () => {
    await applyCrop();
    addToast('success', t('editor_toast_cropped')); // ← t()
  };

  const handleApplyFormat = async () => {
    await applyFormatConversion();
    addToast('success', t('editor_toast_converted', { format: state.outputFormat })); // ← t() with var
  };

  // ─── Download ─────────────────────────────────────────────────────────────
  const handleDownload = () => {
    if (!state.editedDataUrl) {
      addToast('error', t('editor_toast_no_image')); // ← t()
      return;
    }
    downloadEdited();
    addToast('success', t('editor_toast_downloaded')); // ← t()
  };

  // ─── Save to history ──────────────────────────────────────────────────────
  const handleSaveToHistory = async () => {
    if (!user) {
      addToast('info', t('editor_toast_signin')); // ← t()
      return;
    }
    if (!state.originalFile || !state.editedDataUrl || !state.originalDataUrl) {
      addToast('error', t('editor_toast_nothing')); // ← t()
      return;
    }
    ...
    try {
      ...
      addToast('success', t('editor_toast_saved')); // ← t()
    } catch (e) {
      console.error(e);
      addToast('error', t('editor_toast_save_fail')); // ← t()
    } finally {
  ...
  const origExt = state.originalFile ? (state.originalFile.type.split('/')[1] as ImageFormat) ?? 'jpg' : 'jpg';
      ...
      <main className="flex-1 flex flex-col lg:flex-row gap-0 overflow-hidden">
        ...
        >
          ...
          {hasImage && (
            <section>
              <p className="text-xs font-semibold mb-2 tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>
                {t('editor_section_tools')} {/* ← t() */}
              </p>
              <ToolPanel activeTool={state.activeTool} onToolChange={(tool) => setPartial({ activeTool: tool })}>
          ...
          {hasImage && (
            <section className="space-y-2">
              <p className="text-xs font-semibold mb-2 tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>
                {t('editor_section_actions')} {/* ← t() */}
              </p>

              <Button variant="primary" className="w-full" icon={<Download size={14} />} onClick={handleDownload} disabled={!hasEdits && !hasImage}>
                {t('editor_btn_download')} {/* ← t() */}
              </Button>

              {user ? (
                <Button variant="secondary" className="w-full" icon={<Save size={14} />} onClick={handleSaveToHistory} loading={state.isProcessing} disabled={!hasEdits}>
                  {t('editor_btn_save')} {/* ← t() */}
                </Button>
              ) : (
                <Link href="/login" className="block">
                  ...
                  >
                    <Lock size={13} />
                    {t('editor_btn_signin_save')} {/* ← t() */}
                  </button>
                </Link>
              )}

              <Button variant="ghost" size="sm" className="w-full" icon={<RotateCcw size={13} />} onClick={resetEdits}>
                {t('editor_btn_reset')} {/* ← t() */}
              </Button>
            </section>
          )}
        </aside>
        ...
        <section className="flex-1 flex flex-col p-4 gap-3 min-w-0 overflow-hidden">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>
              {t('editor_section_preview')} {/* ← t() */}
            </p>
            {hasImage && (
              <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--text-muted)' }}>
                <span>
                  {t('editor_original')}: <span style={{ color: 'var(--text)' }}>{formatBytes(state.originalSize)}</span> {/* ← t() */}
                </span>
                {hasEdits && (
                  <>
                    <span>→</span>
                    <span>
                      {t('editor_edited')}: <span style={{ color: 'var(--accent)' }}>{formatBytes(state.editedSize)}</span> {/* ← t() */}
                    </span>
                  </>
                )}
              </div>
          ...
          <div
            ...
            style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', minHeight: '320px' }}
          >
            <div className="absolute inset-0 checkerboard opacity-20 rounded-2xl" />

            {state.isProcessing && (
              <div className="absolute inset-0 flex items-center justify-center z-20 rounded-2xl" style={{ background: 'rgba(10,13,20,0.75)', backdropFilter: 'blur(4px)' }}>
                <LoadingSpinner size={32} label={t('editor_processing')} /> {/* ← t() */}
              </div>
            )}
            ...
            {hasImage ? (
              ...
              <div className="relative z-10 flex flex-col items-center gap-3 text-center px-8">
                ...
                <div>
                  <p className="font-semibold" style={{ color: 'var(--text)' }}>{t('editor_no_image_title')}</p> {/* ← t() */}
                  <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>{t('editor_no_image_sub')}</p> {/* ← t() */}
                </div>
              </div>
            )}
          </div>
        ...
        <aside
          ...
          style={{ borderLeft: '1px solid var(--border)', maxHeight: 'calc(100vh - 56px)' }}
        >
          <p className="text-xs font-semibold tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>
            {t('editor_section_compare')} {/* ← t() */}
          </p>

          <BeforeAfterSlider beforeUrl={state.originalDataUrl} afterUrl={hasEdits ? state.editedDataUrl : null} />

          {/* Size comparison bars */}
          {hasEdits && (
            <div className="p-3 rounded-xl space-y-2" style={{ background: 'var(--surface-1)', border: '1px solid var(--border)' }}>
              <p className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>{t('editor_size_comparison')}</p> {/* ← t() */}
              ...
              {state.editedSize < state.originalSize && (
                <p className="text-xs text-right" style={{ color: '#34d399' }}>
                  {t('editor_smaller', { pct: Math.round((1 - state.editedSize / state.originalSize) * 100) })} {/* ← t() with var */}
                </p>
              )}
            </div>
          )}
          ...
          {/* Guest nudge */}
          {!user && hasEdits && (
            <div className="p-3 rounded-xl text-sm" style={{ background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', color: 'var(--text-muted)' }}>
              <p className="font-medium mb-1" style={{ color: '#a78bfa' }}>{t('editor_guest_title')}</p> {/* ← t() */}
              <p className="text-xs">
                <Link href="/register" style={{ color: 'var(--accent)' }} className="hover:underline">
                  {t('editor_guest_link')} {/* ← t() */}
                </Link>{' '}
                {t('editor_guest_body')} {/* ← t() */}
              </p>
            </div>
          )}
        </aside>
